...
class Note(object):
    ...
    def find_shared_notes(cls):
        return [cls(**elem) for elem in Database.find(NoteConstants.COLLECTION, {"shared": True})]

    @classmethod
    def find_shared_notes_by_user(cls, user_email):
        return [cls(**elem) for elem in Database.find(NoteConstants.COLLECTION, {"shared": True, "author_email": user_email})]
